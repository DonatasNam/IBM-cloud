...
/** @file iotp_application.h
    @brief **Application Client Reference** exposes functions and declarations to build IoTP application client.
...
/**
 * The IoTPApplication_setEventCallback() API sets event callback to get notifications on event delivery status.
 *
 * @param application    - A pointer to IoTP application handle.
 * @param cb             - A Function pointer to the IoTPEventCallbackHandler.
 * @return IOTPRC        - Returns IOTPRC_SUCCESS onsuccess or IOTPRC_* on error
 */
DLLExport IOTPRC IoTPApplication_setEventCallback(IoTPApplication *application, IoTPEventCallbackHandler cb);
